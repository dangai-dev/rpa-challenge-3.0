...
from src.infrastructure.settings import SEARCH_TEXT, MONTHS_TO_SEARCH
...
class Scraper:
    ...
    def fetch_posts(self, postsList = list[Post]):
        try:
            ...
            search_until_date = datetime.now() - relativedelta(months=int(months_to_search))
            for post in posts:
                if len(postsList) > 0:
                    last_post_date = datetime.strptime(postsList[-1].date, '%Y-%m-%d').date()
                    if last_post_date.month < search_until_date.month:
                        logging.warning(f'... A DATA QUEBROU AQUI: {last_post_date.month} < {search_until_date.month}...')
                        break
                ...
                date = Helper.convert_to_datetime(date_post) if date_post != "" else date_post
